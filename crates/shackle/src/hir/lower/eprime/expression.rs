...
impl ExpressionCollector<'_> {
    ...
    pub fn collect_expression(&mut self, expression: eprime::Expression) -> ArenaIndex<Expression> {
        ...
        let collected: Expression = match expression {
            ...
            eprime::Expression::InfixOperator(o) => return self.collect_infix_operator(o.operator(), o.left(), o.right(), origin),
			...
			eprime::Expression::SetConstructor(o) =>  return self.collect_infix_operator(o.operator(), o.left(), o.right(), origin)
        };
	...
	pub(super) fn collect_domain_expressions(&mut self, t: eprime::Domain, var_type: VarType) -> CollectedDomain {
		...
		CollectedDomain::BoundedDomain(match t {
			...
			eprime::Domain::DomainOperation(d) => {
				let left = self.collect_domain_expressions(d.left(), var_type).into_expression(self, origin.clone());
				let right = self.collect_domain_expressions(d.right(), var_type).into_expression(self, origin.clone());
				let op = d.operator();
			...
			eprime::Domain::IntegerDomain(i) => {
				...
				let call_domain = if set_constructor_domain_members.len() > 1 {
					...
					set_constructor_domain_members
						.into_iter()
	...
	fn collect_infix_operator(&mut self, operator: eprime::Operator, l: eprime::Expression, r: eprime::Expression, o: Origin) -> ArenaIndex<Expression> {
	...
	pub fn collect_matrix_literal(&mut self, ml: eprime::MatrixLiteral) -> ArenaIndex<Expression> {
				...
				eprime::Expression::MatrixLiteral(ml) => {
					if is_finding_dimensions {
						...
						if let Some(i) = ml.index() {
							index_sets.push(self.collect_domain_expressions(i, VarType::Par).into_expression(self, origin.clone()));
						}
					}
		...
		match (dimensions.len(), index_sets.len()) {
			...
			(2, 0) => return self.alloc_expression(origin, ArrayLiteral2D {
				members,
			...
			(d, i) => {
				...
				if d > 6 {
					self.add_diagnostic(InvalidArrayLiteral{
						src,
						span,
						msg: "Support for matrix literals with >6 dimensions not currently supported".to_string()
					});
					return self.alloc_expression(origin, Expression::Missing);
				}
				if d != i && i != 0 {
				...
				if i == 0 {
					...
					index_sets = dimensions.iter().map(|n| {
						let one = self.alloc_expression(origin.clone(), IntegerLiteral(1));
						let n = self.alloc_expression(origin.clone(), IntegerLiteral(*n as i64));
				...
				return self.alloc_expression(origin, Call {
					function,
	...
	fn collect_quantification(&mut self, q: eprime::Quantification) -> Call {
		...
		let function = self.ident_exp(origin.clone(), match q.function().name() {
			"forAll" => "forall",
			q => q,
		});
		Call {
	...
	fn collect_matrix_comprehension(&mut self, m: eprime::MatrixComprehension) -> Call {
		...
		let generators = m
			...
			.collect();
		let matrix_comprehension = self.alloc_expression(origin.clone(), ArrayComprehension { 
			template,
			indices: None,
			generators,
		});
		...
		match m.indices() {
			Some(i) => {
				let index_set = self.collect_domain_expressions(i, VarType::Par).into_expression(self, origin.clone());
				Call {
	...
	fn collect_generator(&mut self, g: eprime::Generator, where_clause: Option<ArenaIndex<Expression>>) -> Generator {
		...
		let patterns = g
			...
			.collect();
		let collection = self.collect_domain_expressions(g.collection(), VarType::Par).into_expression(self, origin);
		Generator::Iterator { 
			patterns, 
			collection,
			where_clause
...
impl CollectedDomain {
	...
	pub(super) fn into_expression(self, ctx: &mut ExpressionCollector, origin: Origin) -> ArenaIndex<Expression> {
		match self {
			...
			CollectedDomain::PrimitiveDomain(p) => {
				...
				let (l, r): (Expression, Expression) = match p {
					PrimitiveType::Bool => (BooleanLiteral(false).into(), BooleanLiteral(true).into()),
					PrimitiveType::Int => {
						let inf = ctx.alloc_expression(origin.clone(), Expression::Infinity);
						(Call { function: ctx.ident_exp(origin.clone(), "-"), arguments: Box::new([inf]) }.into(), Expression::Infinity.into())
					},
					PrimitiveType::Float  |
					PrimitiveType::String |
					PrimitiveType::Ann => unreachable!("These primatives aren't implemented in EPrime"),
				};
				...
				ctx.alloc_expression(origin, Call {
					function,
