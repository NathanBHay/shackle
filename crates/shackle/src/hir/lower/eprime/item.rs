	...
	fn collect_declarations<I: Iterator<Item = eprime::Identifier>>(
		...
		for name in names {
			...
			let (definition, declared_type) = if domain_is_definition {
				(Some(ctx.collect_domain_expressions(domain.clone(), VarType::Par).into_expression(&mut ctx, origin.clone())),
					ctx.alloc_type(origin.clone(), Type::Set { inst: VarType::Par, opt: OptType::NonOpt, element: declared_type }))
			} else { (None, declared_type) };
